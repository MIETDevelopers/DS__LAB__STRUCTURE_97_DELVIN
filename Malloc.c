...
int delvin=-1,James,i,n; // Variable Declratation   Initially there is no element in stack
...
scanf("%d",&James);
delvin=delvin+1; // "element" is incremented after an element is pushed
stack[delvin]=James;// The pushed element is made as element
}
}
...
{
printf("\nDeleted element is %d",stack[delvin]);
delvin=delvin-1;  //  Decrement element after a pop
}
}
